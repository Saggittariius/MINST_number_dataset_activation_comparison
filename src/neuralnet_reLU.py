...
Uses ReLU activation in hidden layers and output layer with MSE loss.
...
class Network:
    ...
    def backprop(self, 
        ...
        for layer_index, (b, w) in enumerate(zip(self.biases, self.weights)):
            ...
            # Apply activation function to the hidden layers
            if layer_index == len(self.weights)-1:
                activation = z
            else:
                activation = relu(z)
...
def export_metrics(rows: List[dict]) -> None: # Save training metrics to CSV file.
    ...
    out_csv = "nminst_results_relu.csv"
