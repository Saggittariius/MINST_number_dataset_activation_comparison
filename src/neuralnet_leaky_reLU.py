"""
Neural Network with Leaky ReLU Activation
==========================================
Implementation of a feedforward neural network from scratch using NumPy.
Uses Leaky ReLU activation in hidden layers and output layer with MSE loss.
    ...
    def feedforward(self, a: np.ndarray) -> np.ndarray: # Compute network output for input a.
        for layer_index, (b, w) in enumerate(zip(self.biases, self.weights)):
            ...
            if layer_index == len(self.weights)-1:
                 a = leaky_relu(z) # Output layer uses leaky ReLU
            else:
...
def leaky_relu(z: np.ndarray, alpha: float=0.01) -> np.ndarray: # Leaky ReLU activation function.
    return np.where(z>0, z, alpha*z)

def leaky_relu_prime(z: np.ndarray, alpha: float=0.01) -> np.ndarray: # Derivative of Leaky ReLU.
